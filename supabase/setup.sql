...
begin
  ...
  if not exists (
    select 1 from pg_policies
    where schemaname = 'storage' and tablename = 'objects' and policyname = 'profile_photos_select_all'
  ) then
    create policy profile_photos_select_all on storage.objects
      for select using (bucket_id = 'profile-photos');
  end if;

  if not exists (
    select 1 from pg_policies
    where schemaname = 'storage' and tablename = 'objects' and policyname = 'profile_photos_insert_all'
  ) then
  ...
  if not exists (
    select 1 from pg_policies
    where schemaname = 'storage' and tablename = 'objects' and policyname = 'profile_photos_update_all'
  ) then
  ...
  if not exists (
    select 1 from pg_policies
    where schemaname = 'storage' and tablename = 'objects' and policyname = 'profile_photos_delete_all'
  ) then
  ...
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'profiles' and policyname = 'profiles_insert_all'
  ) then
  ...
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'profiles' and policyname = 'profiles_update_all'
  ) then
  ...
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'profiles' and policyname = 'profiles_delete_all'
  ) then
  ...
  if not exists (
    ...
    create policy groups_insert_all on public.groups for insert with check (auth.uid() = owner_id);
  end if;
  ...
  if not exists (
    ...
    create policy groups_update_all on public.groups for update using (auth.uid() = owner_id) with check (auth.uid() = owner_id);
  end if;
  ...
  if not exists (
    ...
    create policy groups_delete_all on public.groups for delete using (auth.uid() = owner_id);
  end if;
  ...
  if not exists (
    ...
    create policy group_members_insert_all on public.group_members for insert with check (auth.uid() = user_id);
  end if;
  ...
  if not exists (
    ...
    create policy group_members_update_all on public.group_members for update using (
      auth.uid() = user_id
      or exists (
        select 1
        from public.groups
  ...
  if not exists (
    ...
    create policy group_members_delete_all on public.group_members for delete using (
      auth.uid() = user_id
      or exists (
        select 1
        from public.groups
